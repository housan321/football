...
def bt_lnkXDat2(g10,kcid):
    ...
    for i, row in g10.iterrows():
        ...
        if os.path.exists(fxdat_oz) and os.path.exists(fxdat_az) and os.path.exists(fxdat_tz):
            ...
            if len(odds_oz) >= tfsys.cidrows and len(odds_az) >= tfsys.cidrows and len(odds_tz) == 1:    #如果数据有CID_ROWS行并且有比赛结果才处理数据
                
                odds_oz = odds_oz[odds_oz['cid'] == '5']
                odds_az = odds_az[odds_az['cid'] == '5']
                ...
                merge_data = pd.concat([odds_az, odds_oz, odds_tz], axis=1)                 
...
#    xdatRR,gid10RR=bt_lnkXDat(g10,xtfb.kcid)   # xtfb.gid10：赛事ID号
    xdat,xtfb.gid10 = bt_lnkXDat2(g10, xtfb.kcid)   # xtfb.gid10：赛事ID号
    if len(xdat.index)>0:
        #--dat.pre0
